...
export async function exercisePatchedLocalAgent(source, {modelId, authority, nativeSetting, prefix = 'chatgpt-codex/'}) {
  ...
  const injected = name => {
    const match = source.match(new RegExp('function ' + name + '\\(existing,[\\s\\S]*?\\n}'));
  ...
  const factory = new Function('__subscriptionSelectedModelIds','__ClaudeSelectedModelIds','__useChatgptDedicatedRuntime','__isClaudeBridgeModel',trim,binary,setting,
    'return ({' + method + '}).runLocalAgentInExtensionHost;');
  const route = factory(injected('__subscriptionSelectedModelIds'),injected('__ClaudeSelectedModelIds'),
  ...
  const calls = [];
  const provider = {
    waitForProviderRegistration: async () => calls.push('registered'),
  ...
  await route.call(service, {signal}, bytes, bytes, model, {}, {}, {}, [], resources,
    {
      subscriptionActionChannel: 'subscription-actions:test',
      subscriptionPlanPrepends: [[1, 2]],
      conversationId: 'synthetic-conversation',
      requestedModel: {...bytes, modelId},
      subagentModelOverrides: [exploreOverride]
    });
  assert.equal(calls[0], 'registered');
  const [kind, request, callbacks, ...rest] = calls[1];
  return {name, kind, request, callbacks, rest, override: exploreOverride, prefix, modelId, resources, signal};
}

export async function verifyWorkbenchRouting(source, prefix = 'chatgpt-codex/') {
  for (const [modelId, authority, nativeSetting, expected] of [
    ...
    const {kind, request, callbacks, rest, resources, signal} = await exercisePatchedLocalAgent(source, {modelId, authority, nativeSetting, prefix});
