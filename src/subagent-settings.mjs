...
export function patchSubagentSettingsWorkbench(source, prefix) {
  ...
  return once(source,match[0],','+match[1]+'=__subscriptionSelectedModelIds('+match[2]+','+match[4]+'.subagentModelOverrides,'+match[4]+'.requestedModel?.modelId??i?.modelId,'+quoted+'),'+match[3]+'=i??this.createDefaultLocalModel('+match[4]+')')
