...
if(wb.includes('const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);')){
  ...
  wb=replaceOnce(wb,'async run(e,t,n,i,r,s,o,a,c,l,u){const h=ZHf(u,{isRunningInTest:u.isRunningInTest??this.environmentService.enableSmokeTestDriver===!0,localMode:qc.localMode});if(qc.localMode){',
   'async run(e,t,n,i,r,s,o,a,c,l,u){const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);const h=ZHf(u,{isRunningInTest:u.isRunningInTest??this.environmentService.enableSmokeTestDriver===!0,localMode:qc.localMode||__chatgptLocal});if(qc.localMode||__chatgptLocal){');
}
wb+=`\n$e(class extends at{constructor(){super({id:"cursor.chatgpt.login",title:{value:"ChatGPT: Sign in (subscription)",original:"ChatGPT: Sign in (subscription)"},f1:!0})}async run(e){try{const r=await fetch(__chatgptBridgeBase+"/login",{method:"POST",headers:{Authorization:"Bearer "+__chatgptBridgeKey}});if(!r.ok)throw new Error("Could not start sign-in");e.get(oi).info("Complete ChatGPT sign-in in your browser, then reload the Cursor window.")}catch(error){e.get(oi).error("Cannot reach the ChatGPT connection. Restart Cursor.")}}});\n`;
wb=addUsage(wb,{fn:'function KIy(e){const t=Pbp(119)',jsx:'HIy',
  original:'title:"Plan & Usage",children:[ln,qt,un,pn]',
  claudeOnly:'title:"Plan & Usage",children:[ln,qt,un,pn,HIy(__claudeUsageSection,{})]',
  symbols:{jsx:'HIy',useState:'Xur',useEffect:'Zxy',card:'sb',zs:'Ts',bar:'yA',barStyle:'idr'}});
wb=patchMaxMode(patchSubagentSettingsWorkbench(wb));
wb=patchSubagentBubbles(patchRemoteRouting(wb,'desktop','3.21.9'),'desktop','3.21.9');
wb=patchSubagentLifecycle(wb,'desktop','chatgpt-codex/','3.21.9');
...
glass=wrapMap(glass,{plain:'const S=d(l);l=l.map(U=>qon(U)),',claude:'const S=d(l);l=__withClaudeBridgeModels(l).map(U=>qon(U)),',gpt:'const S=d(l);l=__withChatgptBridgeModels(l).map(U=>qon(U)),'});
...
if(glass.includes('const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);')){
  ...
  glass=replaceOnce(glass,'async run(t,e,n,i,r,s,o,a,l,c,u){const d=IES(u,{isRunningInTest:u.isRunningInTest??this.environmentService.enableSmokeTestDriver===!0,localMode:Ml.localMode});if(Ml.localMode){',
   'async run(t,e,n,i,r,s,o,a,l,c,u){const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);const d=IES(u,{isRunningInTest:u.isRunningInTest??this.environmentService.enableSmokeTestDriver===!0,localMode:Ml.localMode||__chatgptLocal});if(Ml.localMode||__chatgptLocal){');
}
glass+=`\nOt(class extends Zt{constructor(){super({id:"cursor.chatgpt.login",title:{value:"ChatGPT: Sign in (subscription)",original:"ChatGPT: Sign in (subscription)"},f1:!0})}async run(){const r=await fetch(__chatgptBridgeBase+"/login",{method:"POST",headers:{Authorization:"Bearer "+__chatgptBridgeKey}});if(!r.ok)throw new Error("ChatGPT-Could not start sign-in")}});\n`;
glass=addUsage(glass,{fn:'function Cik(t){const e=gBg(119)',jsx:'Sik',
  original:'title:"Plan & Usage",children:[pt,bt,gt,yt]',
  claudeOnly:'title:"Plan & Usage",children:[pt,bt,gt,yt,Sik(__claudeUsageSection,{})]',
  symbols:{jsx:'Sik',useState:'pds',useEffect:'yik',card:'Rf',zs:'Os',bar:'Om',barStyle:'CEi'}});
glass=patchMaxMode(patchSubagentSettingsWorkbench(glass));
glass=patchSubagentBubbles(patchRemoteRouting(glass,'glass','3.21.9'),'glass','3.21.9');
glass=patchSubagentLifecycle(glass,'glass','chatgpt-codex/','3.21.9');
