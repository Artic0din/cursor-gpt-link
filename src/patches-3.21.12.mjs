...
if(wb.includes('const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);')){
  ...
  wb=replaceOnce(wb,'async run(e,t,n,i,r,s,o,a,c,l,u){const h=a6d(u,{isRunningInTest:u.isRunningInTest??this.environmentService.enableSmokeTestDriver===!0,localMode:jc.localMode});if(jc.localMode){',
   'async run(e,t,n,i,r,s,o,a,c,l,u){const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);const h=a6d(u,{isRunningInTest:u.isRunningInTest??this.environmentService.enableSmokeTestDriver===!0,localMode:jc.localMode||__chatgptLocal});if(jc.localMode||__chatgptLocal){');
}
...
wb=addUsage(wb,{fn:'function ZIy(e){const t=Obp(119)',jsx:'GIy',
  ...
  claudeOnly:'title:"Plan & Usage",children:[ln,qt,un,pn,GIy(__claudeUsageSection,{})]',
  symbols:{jsx:'GIy',useState:'Zur',useEffect:'eIy',card:'sb',zs:'Ts',bar:'yA',barStyle:'rdr'}});
wb=patchMaxMode(patchSubagentSettingsWorkbench(wb));
wb=patchSubagentBubbles(patchRemoteRouting(wb,'desktop','3.21.12'),'desktop','3.21.12');
wb=patchSubagentLifecycle(wb,'desktop','chatgpt-codex/','3.21.12');
...
if(glass.includes('const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);')){
  ...
  glass=replaceOnce(glass,'async run(t,e,n,i,r,s,o,a,l,c,u){const d=wum(u,{isRunningInTest:u.isRunningInTest??this.environmentService.enableSmokeTestDriver===!0,localMode:Al.localMode});if(Al.localMode){',
   'async run(t,e,n,i,r,s,o,a,l,c,u){const __chatgptLocal=__isChatgptBridgeModel(u?.requestedModel?.modelId??i?.modelId);const d=wum(u,{isRunningInTest:u.isRunningInTest??this.environmentService.enableSmokeTestDriver===!0,localMode:Al.localMode||__chatgptLocal});if(Al.localMode||__chatgptLocal){');
}
...
glass=addUsage(glass,{fn:'function Rik(t){const e=fBg(119)',jsx:'xik',
  original:'title:"Plan & Usage",children:[mt,bt,gt,yt]',
  claudeOnly:'title:"Plan & Usage",children:[mt,bt,gt,yt,xik(__claudeUsageSection,{})]',
  symbols:{jsx:'xik',useState:'gds',useEffect:'Eik',card:'Rf',zs:'Os',bar:'Fm',barStyle:'EEi'}});
glass=patchMaxMode(patchSubagentSettingsWorkbench(glass));
glass=patchSubagentBubbles(patchRemoteRouting(glass,'glass','3.21.12'),'glass','3.21.12');
glass=patchSubagentLifecycle(glass,'glass','chatgpt-codex/','3.21.12');
