...
- ChatGPT and Claude subscription models on This Mac (Remote Control) now show a clear error pointing to This Mac, instead of failing in Cursor's cloud create RPC with `BAD_MODEL_NAME`. Remote Control agents must be created in the cloud and the cloud cannot reach the local bridge. Cursor-native models, Cloud and Remote Machine targets are unchanged.
- Stopped installation when the Remote Control guard anchor is missing or duplicated, or when the earlier Remote Control reroute patch is still installed.
