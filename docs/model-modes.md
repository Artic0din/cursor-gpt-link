...
Select the desired window under **Context**. Where Cursor exposes its legacy MAX switch, it selects the larger window and switching it off selects the normal window. On versions that install this patch, the patch preserves the selected effort and Fast value. Models without a larger supported window do not advertise MAX support.
